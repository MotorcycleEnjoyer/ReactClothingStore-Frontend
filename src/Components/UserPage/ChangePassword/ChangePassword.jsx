...
export async function action({ request }) {
    ...
    if (response === 200) {
        const conf = window.confirm(
            "Password change success! Click OK to go to homepage."
        );
        if (conf) {
            window.location = "/";
        }
...
export default function ChangePassword() {
    ...
    const passwordStyle = {
        pw: {
            border:
                password.length >= 8 ? "5px solid green" : "5px solid black",
        },
        cpw: {
            border:
        ...
        <div className="auth">
            ...
            <Form method="post" className="changePassword">
                ...
                {password === confirmPassword &&
                    confirmPassword.length >= 8 && <button>Submit</button>}
            </Form>
        </div>
    );
}
