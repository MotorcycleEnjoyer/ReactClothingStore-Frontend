...
export default function NewCart() {
    ...
    const totalCost = shoppingCart.reduce(
        (accumulator, currentVal) =>
            accumulator + currentVal.details.price * currentVal.amount,
        iniVal
    );
    ...
    function clearCart() {
        const yesIWantToDelete = window.confirm(
    ...
    function activateCartModal(propsFromCartItem) {
        const itemToAppend = (
            <ShoppingProduct
                {...propsFromCartItem}
                modal={true}
                editCartItem={changeCartItem}
                hideCartModal={hideCartModal}
                toggleCartModal={activateCartModal}
                view="fullSize"
            />
        );
    ...
    const cartAsHTML = shoppingCart?.map((item, index) => (
        <ShoppingProduct
            key={index}
            index={index}
            removeFromCart={() => deleteCartItem(index)}
            {...item}
            toggleCartModal={activateCartModal}
            view={"cart"}
        />
        ...
        <>
            {shoppingCart?.length > 0 && (
                <>
                    ...
                    >
                        <div className="cartItem--modal--content">
                            <button
                                ...
                                style={{ padding: "20px", marginRight: "0px" }}
                            >
                                Close Edit Menu
                            </button>
                    ...
                    <div className="cartItems">{cartAsHTML}</div>
